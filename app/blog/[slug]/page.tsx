...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${config.domain}/blog/${slug}` },
    openGraph: {
      title: post.title,
      description: post.description,
      type: 'article',
      publishedTime: post.date,
      images: [{ url: `${config.domain}/images/desktop-banner.webp`, width: 1200, height: 630, alt: post.title }],
    },
    twitter: {
      card: 'summary_large_image',
      title: post.title,
      description: post.description,
      images: [`${config.domain}/images/desktop-banner.webp`],
    },
  };
}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    image: { '@type': 'ImageObject', url: `${config.domain}/images/desktop-banner.webp`, width: 1200, height: 630 },
    author: { '@type': 'Organization', name: 'Укртелеком', url: config.domain },
    publisher: {
      '@type': 'Organization',
      name: 'Укртелеком',
      logo: { '@type': 'ImageObject', url: `${config.domain}/images/logo.svg` },
    },
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: post.title, item: `${config.domain}/blog/${slug}` },
    ],
  };
